...
(function($, window, document, undefined) {
    $.fn.socialfeed = function(options) {
        ...
        var options = $.extend(defaults, options),
            container = $(this),
            template,
            social_networks = ['facebook', 'instagram', 'vk', 'google', 'blogspot', 'twitter', 'flickr'];
        ...
        var Feed = {
            ...
            flickr: {
              ...
              getData: function(account) {
                var account = options.flickr.accounts[0],
                    lang = "en-en",
                    format = "json"
                    url = 'https://api.flickr.com/services/feeds/photos_public.gne?user_id=' + account+ '&lang=' + lang + '&format=' + format + '&callback=?';


                Utility.request(url, Feed.flickr.utility.getPublicPhotos);

                // Flickr feed api enforces that the callback is named jsonFlickrFeed...
                jsonFlickrFeed = function(data) {
                  return Feed.flickr.utility.getPublicPhotos(data);
                };

              },
              utility: {
                getPublicPhotos: function(json) {
                  ...
                  items.forEach(function(element, index) {
                    element.id = index;
                    var post = new SocialFeedPost('flickr', Feed.flickr.utility.unifyPostData(element));
                    console.log(post);
                    post.render();
                  });
                },
                unifyPostData: function(element) {
                  ...
                  post.id = element.id;
                  post.dt_create = moment(element.date_taken * 1000);
                  post.author_link = 'http://flickr.com/' + element.author_id;
                  post.author_picture = 'https://s.yimg.com/pw/images/buddyicon10_l.png#' + element.author_id;
                  post.author_name = element.author;
                  post.message = '';
                  post.description = Utility.stripHTML(element.description);
                  post.social_network = 'flickr';
                  post.link = element.link;
                  if (options.show_media) {
                    post.attachment = '<img class="attachment" src="' + element.media.m + '" />';
                  }
                  return post;
                }
              }
